...
Four routes and one table column. The browser asks for options, the
...
async def register_options(request: Request) -> dict[str, Any]:
    ...
    async with db.untenanted() as connection:
        ...
        challenge = _random()
        await connection.execute(_SAVE_CHALLENGE, session.user_id, challenge)
...
async def register_verify(request: Request) -> dict[str, Any]:
    ...
    async with request.app.state.db.untenanted() as connection:
        challenge = await connection.fetchval(_TAKE_CHALLENGE, session.user_id)
        if not challenge:
...
async def login_options(request: Request) -> dict[str, Any]:
    ...
    async with db.untenanted() as connection:
        ...
        challenge = _random()
        await connection.execute(_SAVE_CHALLENGE, user_id, challenge)
...
async def login_verify(request: Request, response: Response) -> dict[str, Any]:
    ...
    async with db.untenanted() as connection:
        ...
        challenge = await connection.fetchval(_TAKE_CHALLENGE, user_id)
        if not challenge:
            raise ApiError(400, "no_challenge", "request options first")
