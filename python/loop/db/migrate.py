...
def default_migrations_dir() -> Path:
    """`packages/db/migrations/` — shared with the reference while both run.

    The schema is the one thing the two implementations cannot each keep a copy
    of: they talk to the same database, and two sources of truth for a
    constraint is how a differential stops meaning anything. It moves under
    `python/` when the TypeScript retires.
    """
