...
async def stats(request: Request, period: str = _DEFAULT_PERIOD) -> dict[str, Any]:
    ...
    async with request.app.state.db.session(
        session.user_id, read_only=True
    ) as connection:
...
async def stats_payload(
    ...
    channels = await connection.fetch(_channels_sql(), user_id)
    ...
    by_month = await connection.fetch(_by_month_sql(_WINDOW[window]), user_id)
    outcomes = await connection.fetchrow(_outcomes_sql(_REACH_WINDOW[window]), user_id)
